...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch("https://bakad.onrender.com/api/v1/auth/login", {
        method: "POST",
        ...
        <div className="glass-card rounded-[2rem] p-10 md:p-14 w-full border-gold">
          <div className="text-center mb-10">
            ...
            <p className="text-gray-500 font-medium">
              Access the Bank Ads Prestige Engine
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="flex items-center justify-between text-sm">
              <label className="flex items-center text-gray-500 hover:text-white cursor-pointer transition-colors font-medium">
                <input
                  type="checkbox"
                  className="mr-3 rounded border-gold-500/30 bg-transparent text-gold-500 focus:ring-gold-500/50 h-5 w-5"
                />
                Lock Session
              </label>
              <a
                href="#"
                className="text-gold-500 hover:text-gold-400 transition-colors font-bold"
              >
                Lost Access?
              </a>
            </div>
          ...
          <p className="mt-10 text-center text-gray-500 font-medium">
            ...
            <Link
              to="/register"
              className="text-gold-500 font-black hover:text-gold-400 transition-colors"
            >
              Apply for Access
            </Link>
          </p>
